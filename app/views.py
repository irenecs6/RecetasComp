...
def receta_detalle(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            except IntegrityError:  #porque la BD lanza esta excepcion cuando intetamos guardar un ingrediente que ya existe
                ingrediente_repetido = form.cleaned_data.get('ingrediente') #obtengo el nombre del ingrediente que se quiere añadir
                error_mensaje=f'El ingrediente {ingrediente_repetido} ya esta introducido en la receta.'
                form.add_error(None, error_mensaje) #añadimos el error al formulario
